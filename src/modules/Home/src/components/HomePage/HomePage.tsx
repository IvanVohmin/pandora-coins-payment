...
const HomePage = ({ products }: HomePageProps) => {
  ...
  return (
    <>
        ...
        {filteredProducts.map((product, idx) => (
          <Card key={product.id} className="overflow-hidden hover:shadow-md transition-shadow duration-300">
            ...
            <CardFooter className="pt-2 flex justify-between items-center">
              ...
              <Button onClick={() => setItemChoosed({ show: true, item: product })} className="bg-primary hover:bg-primary/90">
                Выбрать
              </Button>
            </CardFooter>
          </Card>
      ...
      <Dialog open={itemChoosed.show} onOpenChange={() => setItemChoosed({ show: false, item: null })}>
        <DialogContent>
          <DialogHeader>
